...
def banking():
    ...
    chestY = random.uniform(chestdimensions[1]+(0.1*chestdimensions[2]), (chestdimensions[1]+(chestdimensions[3])*0.9))
    dragdur = random.normalvariate(1, 0.5)
...
def depositall():
    ...
    depositdimensions = pyautogui.locateOnScreen(
        'E:\\Desktop\\pycodes - kopie\\fletching\\depositall.png')
    depositx = random.uniform(
        depositdimensions[0], depositdimensions[0]+(depositdimensions[2])*0.9)
    deposity = random.uniform(
        depositdimensions[1], depositdimensions[1]+(depositdimensions[3])*0.9)
    dragdur = random.normalvariate(1, 0.5)
...
def withdrawlogs():
    ...
    logx = random.uniform(
        logdimensions[0], logdimensions[0]+(logdimensions[2])*0.9)
    logy = random.uniform(
        logdimensions[1], logdimensions[1]+(logdimensions[3])*0.9)
    dragdur = random.normalvariate(1, 0.5)
...
def startfletch():
    ...
    knifex = random.uniform(
        knifedimensions[0], knifedimensions[0]+(knifedimensions[2])*0.9)
    knifey = random.uniform(
        knifedimensions[1], knifedimensions[1]+(knifedimensions[3])*0.9)
    dragdur = random.normalvariate(1, 0.5)
